...
// ---------- Komik reading history ----------
// Same reasoning as novel history above — komik chapter URLs don't fit
// AnimeCard's donghua/anime routing, so this stays in its own key.

export function getKomikHistory() {
  return read(KEYS.komikHistory, []);
}

export function pushKomikHistory(item) {
  const current = getKomikHistory().filter((h) => h.chapterUrl !== item.chapterUrl);
  const next = [{ ...item, readAt: Date.now() }, ...current].slice(0, 40);
  write(KEYS.komikHistory, next);
}

export function clearKomikHistory() {
